...
CREATE POLICY stade_full_debrief_photos ON debrief_photos FOR ALL TO authenticated
  USING (is_stade()) WITH CHECK (is_stade());
DROP POLICY IF EXISTS responsable_insert_photos ON debrief_photos;
CREATE POLICY responsable_insert_photos ON debrief_photos FOR INSERT TO authenticated
  WITH CHECK (app_role() = 'ROLE_RESPONSABLE' AND space_id = app_space_id());
DROP POLICY IF EXISTS responsable_read_photos ON debrief_photos;
CREATE POLICY responsable_read_photos ON debrief_photos FOR SELECT TO authenticated
...
CREATE POLICY responsable_delete_photos ON debrief_photos FOR DELETE TO authenticated
  USING (app_role() = 'ROLE_RESPONSABLE' AND space_id = app_space_id());

-- ── Bucket Storage (privé, 10 Mo, images) ──────────────────────────────────
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES ('debrief-photos','debrief-photos', false, 10485760,
        ARRAY['image/jpeg','image/png','image/webp','image/heic'])
ON CONFLICT (id) DO NOTHING;

DROP POLICY IF EXISTS upload_debrief_photo ON storage.objects;
CREATE POLICY upload_debrief_photo ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'debrief-photos');
DROP POLICY IF EXISTS read_debrief_photo ON storage.objects;
CREATE POLICY read_debrief_photo ON storage.objects FOR SELECT TO authenticated
  USING (bucket_id = 'debrief-photos');
DROP POLICY IF EXISTS delete_debrief_photo ON storage.objects;
CREATE POLICY delete_debrief_photo ON storage.objects FOR DELETE TO authenticated
  USING (bucket_id = 'debrief-photos' AND (is_stade() OR app_role() = 'ROLE_RESPONSABLE'));
