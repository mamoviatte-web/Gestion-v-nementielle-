-- =====================================================================
-- ZONE DE DERNIÈRE MINUTE — OPTION « FORMAT BUVETTE » (volume public)
-- ---------------------------------------------------------------------
-- Un bar (ex. Club 70 Sud/Nord) peut être exploité comme une buvette
-- publique sur un match : sa fiche runner doit alors être dotée sur la base
-- « grand public » (ratio affluence GP) et non sur la base pax VIP/Bar.
--
-- generate_runner_dotations reçoit un 3e paramètre optionnel p_as_buvette.
-- Quand il vaut true (avec p_space_id ciblé), la zone est dotée comme une
-- buvette : socle calculé sur le ratio grand public, et complément historique
-- VIP/Bar désactivé pour cette zone. p_as_buvette=false (défaut) = inchangé.
--
-- activate_zone_runner(event, space, p_as_buvette) propage l'option.
-- Idempotent ; ne touche que la zone ciblée.
-- =====================================================================

-- Remplace la signature 2-arg par la version à 3 arguments (défaut false).
drop function if exists public.generate_runner_dotations(uuid, uuid);

CREATE OR REPLACE FUNCTION public.generate_runner_dotations(p_event_id uuid, p_space_id uuid default null, p_as_buvette boolean default false)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  select
    ...
    coalesce(cat.avg_consumption,0),
    case when (s.service_type='buvette' or coalesce(p_as_buvette,false)) then round(v_r_gp,2) else round(coalesce(es.fill_ratio,1.0),2) end,
    reco.q,
  ...
  cross join lateral (
    select ceil(
      ...
      * case when (s.service_type='buvette' or coalesce(p_as_buvette,false)) then v_r_gp else coalesce(es.fill_ratio,1.0) end
      * 1.20
    )::int as q
  ) reco
  where es.event_id=p_event_id and (p_space_id is null or s.space_id=p_space_id)
  ...
  join space_product_catalog cat
       ...
       and not coalesce(p_as_buvette,false)
...
-- ── Wrapper : activer une zone (+ format buvette optionnel) ───────────
create or replace function public.activate_zone_runner(
  p_event_id   uuid,
  p_space_id   uuid,
  p_as_buvette boolean default false
) returns json
language plpgsql
security definer
set search_path to 'public'
...
begin
  ...
  perform generate_runner_dotations(p_event_id, p_space_id, coalesce(p_as_buvette, false));
  ...
  return json_build_object(
    ...
    'as_buvette', coalesce(p_as_buvette, false),
    'newly_linked', v_added, 'lines', v_lines, 'qty_to_move', v_qtm
  );
end;
$function$;

-- Ancienne signature 2-arg éventuelle → on garde la 3-arg (défaut) comme unique.
drop function if exists public.activate_zone_runner(uuid, uuid);
grant execute on function public.activate_zone_runner(uuid, uuid, boolean) to authenticated;
