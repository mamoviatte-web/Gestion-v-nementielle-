-- Fiche runner Loges — dotation par loge = STOCK DE BASE FIXE
-- ============================================================
-- Correction du schéma de calcul de la passe (0) LOGES.
--
-- Règle métier (utilisateur) : « la dotation par loge est systématiquement un
-- stock de base ; cette dotation, quoiqu'il arrive, ne bougera jamais. On
-- soustrait uniquement le stock déjà présent dans l'espace ; s'il n'y a pas de
-- stock, c'est l'intégralité de la dotation des loges qu'il faut monter. »
--
-- Avant : besoin loge = ceil( Σ dotation_loge × fill_ratio )  → l'affluence
--         (fill_ratio) pouvait RÉDUIRE la dotation fixe sous son socle.
-- Après : besoin loge = Σ dotation_loge (fixe), à_monter = max(besoin − stock
--         espace, 0). L'affluence n'entre plus dans la dotation des loges.
--
-- Rappel : la somme est faite PAR PRODUIT (group by space_id, product_id), donc
-- deux blancs distincts (ex. Blanc Montaurone réparti sur 10 loges = 20, Blanc
-- Galiniere sur 4 loges = 9) donnent bien DEUX lignes « à acheminer » (20 et 9),
-- couvrant les 14 loges — et non un seul blanc sous-dimensionné.
