-- Fiche runner — CONSOLIDATION ANTI-MÉLANGE des gammes exclusives
-- ================================================================
-- Aligne le dépôt sur la fonction déjà déployée en base de production.
--
-- Problème : sur un espace VIP (ex. Salon Nord), la fiche runner affichait
-- plusieurs produits d'une même gamme (ex. 3 vins rouges) car les blocs (1)
-- SOCLE et (2) COMPLÉMENT HISTORIQUE génèrent une ligne par produit ayant un
-- référentiel / un historique de conso — sans jamais consulter le produit
-- PRIMAIRE choisi pour l'événement (event_area_product_selection).
--
-- Correctif : bloc (3) CONSOLIDATION ajouté en fin de generate_runner_dotations.
-- Pour chaque (espace, gamme exclusive = product_selection_groups.allow_multiple
-- = false : vin rouge/blanc/rosé, champagne) où plusieurs produits ont été
-- générés :
--   • produit conservé = primaire de l'event (is_primary), sinon dominant
--     historique (plus forte historical_avg_consumption) ;
--   • quantité = SOMME des besoins de la gamme dans l'espace ;
--   • à monter = max(somme_besoin − somme_stock_espace, 0) ;
--   • coût HT recalculé sur le prix du produit conservé × quantité consolidée ;
--   • lignes en double supprimées → une seule ligne par gamme exclusive.
--
-- Garde-fous : seules les lignes validation_status='brouillon' sont touchées ;
-- les Loges sont exclues (leur loge_dotations = dotation manuelle qui fait
-- autorité) ; les gammes non exclusives (allow_multiple=true : softs, eaux,
-- sirops, fûts…) conservent plusieurs produits. Préserve le correctif (0) LOGES
-- (dotation de base fixe, hors affluence).
--
-- Vérifié (Salon Nord — Agen) : Rouge Gigondas 20 (=9+2+9), Rosé Pey Blanc 24
-- (=15+9), Blanc du Seuil 31 (=2+17+12), Mumm Cordon Rouge 36 ; softs = 7 refs
-- conservées ; aucune gamme exclusive en double hors loges.
