-- Fiches runner : (1) liaison stock office prise en compte pour TOUS les espaces
-- (correction : la soustraction n'etait faite que pour retains_stock=true, donc
-- un inventaire en bar/VIP non-retains etait ignore) ; (2) les Loges (Est,
-- Ouest Nord, Ouest Sud) sont redigees directement dans la fiche runner du match
-- a partir de la dotation par loge (somme des dotations individuelles) x facteur
-- d'affluence, moins le stock deja en office. Process : tendance/dotation ->
-- analyse stock espace -> soustraction -> a monter.
