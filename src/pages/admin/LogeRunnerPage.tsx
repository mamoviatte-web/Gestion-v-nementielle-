...
import { Printer, Download, Boxes, LayoutGrid } from 'lucide-react';
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 print:hidden">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm text-stone-400">Dotation par loge individuelle + stockage : « à monter » = total − ce qui est déjà en office.</p>
          </div>
        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-stone-100 text-left text-xs uppercase tracking-wide text-stone-400">
                    ...
                    <th className="px-3 py-2 text-right">Total (dotation)</th>
                    <th className="px-3 py-2 text-right">En office</th>
                ...
                <tbody className="divide-y divide-stone-50">
                  {sheet.synthese.map((r) => (
                    <tr key={r.produit} className="text-stone-800">
                      ...
                      <td className="px-3 py-2 text-right tabular-nums text-stone-500">{r.en_office}</td>
                ...
                <tfoot>
                  <tr className="border-t border-stone-200 bg-stone-50 font-bold text-stone-900">
                    ...
                    <td className="px-3 py-2 text-right tabular-nums">{totals.office}</td>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 print:grid-cols-3">
            {sheet.loges.map((l) => (
              <div key={l.loge} className="overflow-hidden rounded-2xl border border-stone-100 bg-white break-inside-avoid">
                <div className="border-b border-stone-100 bg-amber-50 px-3 py-1.5 text-sm font-bold text-stone-800">{l.loge}</div>
