...
const STATUS: Record<St, { border: string; badge: string; text: string }> = {
  todo: { border: 'border-pr-stone', badge: '⬜ À faire', text: 'text-pr-black-soft/45' },
...
export default function BuvetteSupervisorPage() {
  ...
  return (
    <div className="min-h-screen bg-pr-cream pb-10">
      ...
      {view === 'selection' && (
        <div className="mx-auto max-w-sm space-y-4 p-4">
          <div className="pt-2 text-center">
            {supervisor && <p className="text-xs font-semibold uppercase tracking-wide text-pr-olive-dark">{supervisor}</p>}
            <p className="text-lg font-bold text-pr-black">Quelles buvettes gérez-vous ce soir ?</p>
            <p className="mt-1 text-sm text-pr-black-soft/45">Sélectionnez une ou plusieurs buvettes</p>
          </div>
            ...
            {all.map((b) => {
              ...
              return (
                <button
                  ...
                  className={`flex aspect-square flex-col items-center justify-center gap-1 rounded-2xl border-2 px-1.5 text-center transition-all active:scale-95 ${on ? 'scale-105 border-pr-black bg-pr-black text-white shadow-lg' : 'border-pr-stone bg-white text-pr-black-soft/80 hover:border-pr-stone'}`}
                >
                  <span className={`text-sm font-black leading-tight ${on ? 'text-white' : 'text-pr-black'}`}>{b.label}</span>
          ...
          {selected.size > 0 && (
            <div className="flex items-center gap-3 rounded-xl bg-white px-4 py-3 ring-1 ring-pr-stone">
              <span className="text-xl">🍺</span>
              <div className="flex-1">
                <p className="text-sm font-semibold text-pr-black-soft/90">
                  {selected.size} buvette{selected.size > 1 ? 's' : ''} sélectionnée{selected.size > 1 ? 's' : ''}
                </p>
                <p className="text-xs text-pr-black-soft/45">{all.filter((b) => selected.has(b.code)).map((b) => b.label).join(' · ')}</p>
              </div>
            </div>
          )}
          ...
          <button
            ...
            className="min-h-[56px] w-full rounded-2xl bg-pr-black py-4 text-base font-bold text-white disabled:opacity-40"
          >
      ...
      {view === 'dashboard' && (
        <div className="mx-auto max-w-sm space-y-3 p-4">
          <div className="rounded-2xl border border-pr-stone bg-white px-4 py-3">
            <div className="mb-2 flex items-center justify-between text-sm">
              <span className="font-semibold text-pr-black-soft/80">VOS BUVETTES</span>
              <button onClick={() => setView('selection')} className="rounded-lg border border-pr-stone px-2.5 py-1 text-xs text-pr-black-soft/50 hover:bg-pr-cream">
                Modifier
              </button>
            </div>
            <div className="h-2.5 overflow-hidden rounded-full bg-pr-stone/50">
              <div className="h-full rounded-full bg-green-500 transition-all" style={{ width: mine.length ? `${(done / mine.length) * 100}%` : '0%' }} />
            </div>
            <div className="mt-2 flex gap-3 text-xs">
              <span className="text-pr-black-soft/45">{done}/{mine.length} terminées (stock + débrief)</span>
          ...
          {mine.map((b) => {
            ...
            return (
              ...
              >
                <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-pr-black text-xl text-white">🍺</span>
                <div className="min-w-0 flex-1">
                  <p className="font-bold text-pr-black">{b.label}</p>
                  <span className={`text-xs font-semibold ${cfg.text}`}>{cfg.badge}</span>
                  <div className="mt-1 flex flex-wrap gap-1.5">
                    <span className={`rounded-md px-1.5 py-0.5 text-[10px] font-semibold ${b.has_final ? 'bg-green-100 text-green-700' : b.has_initial ? 'bg-amber-100 text-amber-700' : 'bg-pr-stone/50 text-pr-black-soft/45'}`}>
                      {b.has_final ? '📦 Stock clôturé' : b.has_initial ? '📦 Stock en cours' : '📦 Stock à faire'}
                    </span>
                    <span className={`rounded-md px-1.5 py-0.5 text-[10px] font-semibold ${b.has_debrief ? 'bg-green-100 text-green-700' : 'bg-pr-stone/50 text-pr-black-soft/45'}`}>
          ...
          <button
            ...
            className="flex w-full items-center gap-3 rounded-2xl border border-pr-stone bg-white px-4 py-3.5 text-left"
          >
            <span className="text-xl">⏱</span>
            <div>
              <p className="text-sm font-semibold text-pr-black-soft/90">Horaires de l'équipe</p>
              <p className="text-xs text-pr-black-soft/45">Recenser et valider les heures</p>
            </div>
            <span className="ml-auto text-pr-black-soft/30">›</span>
          </button>
        </div>
      )}
    </div>
