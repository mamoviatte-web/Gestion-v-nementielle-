    ...
    <div className="mb-5 rounded-xl border border-pr-stone bg-white p-4">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="text-xs text-pr-black-soft/45">
          ...
          <tbody>
            {spaces.map((s) => {
              ...
              return (
                <tr key={s.space_id} className="border-t border-pr-stone/60">
                  <td className="py-2">
                    <span className="flex items-center gap-2">
                      <span className={`h-2 w-2 rounded-full ${SERVICE_DOT[s.service_type ?? ''] ?? 'bg-pr-stone'}`} />
                      <span className="font-medium text-pr-black-soft/90">{s.space_name}</span>
                    </span>
                    {s.last_staff_name && <span className="ml-4 text-xs text-pr-black-soft/45">{s.last_staff_name}</span>}
                  </td>
                  ...
                  <td className="py-2 text-right font-medium text-pr-black-soft/80">{s.total_consumed_est ?? 0}</td>
                  <td className="py-2 text-center text-pr-black-soft/70">
                    {s.schedules_done}/{s.schedules_total}
                  </td>
                  <td className="py-2 text-right text-xs text-pr-black-soft/45">{relTime(s.last_activity)}</td>
                </tr>
              );
            })}
          </tbody>
